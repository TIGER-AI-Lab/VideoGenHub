from src.videogen_hub.pipelines.cogvideo.cogvideo_src.cogvideo_pipeline import (
    InferenceModel_Interpolate, InferenceModel_Sequential, 
    my_filling_sequence, get_masks_and_position_ids_stage1,
    get_masks_and_position_ids_stage2,
    my_save_multiple_images
)
from icetk import icetk as tokenizer
from src.videogen_hub.pipelines.cogvideo.cogvideo_src.coglm_strategy import CoglmStrategy
from src.videogen_hub.pipelines.cogvideo.cogvideo_src.sr_pipeline import DirectSuperResolution
...
def pipeline(args, raw_text, height, width, duration):
    ...
    def process_stage2(model, seq_text, duration, video_raw_text=None, video_guidance_text="视频", parent_given_tokens=None, conddir=None, outputdir=None, gpu_rank=0, gpu_parallel_size=1):
        ...
        for sample_i in range(sample_num):
            ...
            for frame_i in range(frame_num_per_sample):
                ...
                decoded_sr_imgs.append(torch.nn.functional.interpolate(decoded_sr_img, size=(height, width)))
    ...
    def process_stage1(model, seq_text, duration, video_raw_text=None, video_guidance_text="视频", image_text_suffix="", outputdir=None, batch_size=1):
        ...
        for seq in output_tokens:
            decoded_imgs = [torch.nn.functional.interpolate(tokenizer.decode(image_ids=seq.tolist()[i*400: (i+1)*400]), size=(height, width)) for i in range(total_frames)]
    ...
    if args.stage_1 or args.both_stages:
        ...
        while True:
            ...
            try:
                path = os.path.join(args.output_path, f"{now_qi}_{raw_text}")
                parent_given_tokens, imgs = process_stage1(model_stage1, raw_text, duration=4.0, video_raw_text=raw_text, video_guidance_text="视频",
                                                     image_text_suffix=" 高清摄影",
                ...
                if args.both_stages:
                    videos = process_stage2(model_stage2, raw_text, duration=duration, video_raw_text=raw_text+" 视频", 
                            video_guidance_text="视频", parent_given_tokens=parent_given_tokens, 
                            outputdir=path,
                            gpu_rank=0, gpu_parallel_size=1) # TODO: 修改
                    return videos
