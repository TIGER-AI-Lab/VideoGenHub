...
class FrameConditioning:
    ...
    def attach_video_frames(self, pl_module, z_0: torch.Tensor = None, batch: torch.Tensor = None,
        ...
        with torch.no_grad():
            ...
            assert n_frames_inference == z_0.shape[
                1], "For frame injection, the number of frames sampled by the dataloader must match the number of frames used for video generation"
            ...
            if getattr(pl_module.opt_params.noise_decomposition, "use", False) and random_mask:
                assert x0.shape[
                           0] == 1, "randomizing frame injection with noise decomposition not implemented for batch size >1"
        return x0, frame_idx
...
class NoiseDecomposition:
    ...
    def __init__(self,
                 ...
                 reduction: str = 'mean'  # added reduction parameter
                 ):
        self.use = use
    ...
    def get_loss(self, x0, unet_base, unet, noise_scheduler, frame_idx, z_t_base, timesteps, encoder_hidden_states,
                 base_noise, z_t_residual, composed_noise):
        if x0 is not None:
            ...
            x0_residual = repeat(x0[:, 0], "B C W H -> B F C W H", F=x0.shape[1] - 1)
        else:
            x0_residual = None

        if self.use_base_model:
            base_pred = unet_base(z_t_base, timesteps, encoder_hidden_states, x0=x0_base).sample
        else:
            base_pred = base_noise

        timesteps_alphas = [noise_scheduler.alphas_cumprod[t.cpu()] for t in timesteps]
        timesteps_alphas = torch.stack(timesteps_alphas).to(base_pred.device)
        timesteps_alphas = repeat(timesteps_alphas, "B -> B F C W H",
                                  F=base_pred.shape[1], C=base_pred.shape[2], W=base_pred.shape[3],
                                  H=base_pred.shape[4])
        base_correction = math.sqrt(self.lambda_f) * torch.sqrt(1 - timesteps_alphas) * base_pred

        z_t_residual_dash = z_t_residual - base_correction

        residual_pred = unet(z_t_residual_dash, timesteps, encoder_hidden_states, x0=x0_residual).sample
        composed_pred = math.sqrt(self.lambda_f) * base_pred.detach() + math.sqrt(1 - self.lambda_f) * residual_pred

        loss_residual = F.mse_loss(composed_noise.float(), composed_pred.float(), reduction=self.reduction)
        if self.use_base_model:
            loss_base = F.mse_loss(base_noise.float(), base_pred.float(), reduction=self.reduction)
    ...
    def add_noise(self, z_base, base_noise, z_residual, composed_noise, noise_scheduler, timesteps):
        z_t_base = noise_scheduler.add_noise(z_base, base_noise, timesteps)
        z_t_residual = noise_scheduler.add_noise(z_residual, composed_noise, timesteps)
    ...
    def split_latent_into_base_residual(self, z_0, pl_module, noise_generator):
        if self.random_frame:
            ...
            fr_select = torch.randint(0, z_0.shape[1], (z_0.shape[0],), device=pl_module.device).long()
            z_base = z_0[:, fr_select, None]
            fr_residual = [fr for fr in range(z_0.shape[1]) if fr != fr_select]
            z_residual = z_0[:, fr_residual, None]
        else:
            frame_idx = list(range(z_0.shape[1]))  # fixed undefined frame_idx
            if not pl_module.unet_params.frame_conditioning.randomize_mask:
                ...
                for batch_idx, frame_at_batch in enumerate(frame_idx):
                    z_base.append(z_0[batch_idx, frame_at_batch, None, None])
                ...
                for batch_idx, frame_idx_batch in enumerate(frame_idx):
                    ...
                    for frame in range(z_0.shape[1]):
                        if frame_idx_batch != frame:
                            z_residual_batch.append(z_0[batch_idx, frame, None, None])
                    z_residual_batch = torch.cat(z_residual_batch, dim=1)
                    z_residual.append(z_residual_batch)
                z_residual = torch.cat(z_residual, dim=0)

        base_noise = noise_generator.sample_noise(z_base)
        residual_noise = noise_generator.sample_noise(z_residual)
        composed_noise = math.sqrt(self.lambda_f) * base_noise + math.sqrt(1 - self.lambda_f) * residual_noise
