...
try:
    from videogen_hub.pipelines.opensora_plan.opensora.models.diffusion.utils.curope import \
        curope as _kernels  # run `python setup.py build_ext --inplace`
except:
    full_dirname = os.path.abspath(os.path.dirname(__file__))
    setup_script = os.path.join(full_dirname, 'setup.py')
    print(f"Failed to import curope kernels, please run 'python {setup_script}' to compile the kernels")
