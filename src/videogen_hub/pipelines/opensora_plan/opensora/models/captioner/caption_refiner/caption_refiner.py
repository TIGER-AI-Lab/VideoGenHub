...
class CaptionRefiner():
    ...
    def video_load_split(self, video_path=None):
        ...
        if ".gif" in video_path:
            ...
            for frame in ImageSequence.Iterator(img):
                frame_np = np.array(frame.copy().convert('RGB').getdata(), dtype=np.uint8).reshape(frame.size[1],
                                                                                                   frame.size[0], 3)
    ...
    def caption_refine(self, video_path, org_caption, model_path):
        ...
        if self.ADD_DETECTION_OBJ:
            ...
            sc_response = model.generate_response(qas, sampled_imgs[0], max_gen_len=1024, temperature=0.9, top_p=0.5,
                                                  seed=0)
        ...
        if self.ADD_ATTR:
            ...
            for obj in existing_objects:
                ...
                for img in sampled_imgs:
                    qas = [
                        ["Please describe the attribute of the {}, including color, position, etc".format(obj), None]]
                    response = model.generate_response(qas, img, max_gen_len=1024, temperature=0.9, top_p=0.5, seed=0)
        ...
        if self.ADD_POS:
            ...
            for obj_pair in obj_pairs:
                qas = [[
                           "What is the spatial relationship between the {} and the {}? Please describe in lease than twenty words".format(
                               obj_pair[0], obj_pair[1]), None]]
                response = model.generate_response(qas, img, max_gen_len=1024, temperature=0.9, top_p=0.5, seed=0)
