...
from videogen_hub.pipelines.lavie.lavie_src.interpolation.utils import mask_generation_before
    ...
    if input_path is not None:
        ...
        if os.path.isdir(input_path):
            ...
            if extention == '.mp4':
                ...
                video_frames = torch.from_numpy(video_reader.get_batch(frame_indice).asnumpy()).permute(0, 3, 1,
                                                                                                        2).contiguous()
                video_frames = transform_video(video_frames)
                n = 3
                del video_reader
...
def auto_inpainting_copy_no_mask(args, video_input, prompt, vae, text_encoder, diffusion, model, device, ):
    ...
    if args.use_ddim_sample_loop:
        samples = diffusion.ddim_sample_loop(
            model.forward_with_cfg, z.shape, z, clip_denoised=False, model_kwargs=model_kwargs, \
            progress=True, device=device, mask=None, x_start=copied_video, use_concat=args.use_concat,
        ...
        for ckpt in [ckpt_path]:
            ...
            for input_path in args.input_path_list:
                ...
                if args.copy_no_mask:
                    ...
                    mask = mask_generation_before(args.mask_type, video_input.shape, video_input.dtype,
                                                  device)  # b,f,c,h,w
                ...
                for idx, prompt in enumerate(class_name):
                    if idx == 0:
                        if args.copy_no_mask:
                            video_clip = auto_inpainting_copy_no_mask(args, video_input, prompt, vae, text_encoder,
                                                                      diffusion, model, device, )
                            video_clip_ = video_clip.unsqueeze(0)
                            all_video.append(video_clip_)
                        else:
                            video_clip = auto_inpainting(args, video_input, masked_video, mask, prompt, vae,
                                                         text_encoder, diffusion, model, device, )
                            video_clip_ = video_clip.unsqueeze(0)
                            all_video.append(video_clip_)
                    else:
                        raise NotImplementedError
                        masked_video = video_input * (mask == 0)
                        video_clip = auto_inpainting_copy_no_mask(args, video_clip.unsqueeze(0), masked_video, mask,
                                                                  prompt, vae, text_encoder, diffusion, model, device, )
                        video_clip_ = video_clip.unsqueeze(0)
                        all_video.append(video_clip_[:, 3:])
                    video_ = ((video_clip * 0.5 + 0.5) * 255).add_(0.5).clamp_(0, 255).to(
                        dtype=torch.uint8).cpu().permute(0, 2, 3, 1)
