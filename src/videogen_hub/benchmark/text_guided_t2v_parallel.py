...
def infer_text_guided_vg_bench(
    ...
    Args:
        ...
        total_portions (int, optional): Total number of portions to divide the dataset into. Defaults to 1.
        portion_index (int, optional): Index of the portion to process (0-based). Defaults to 0.
    ...
    # Divide the prompts into portions
    total_prompts = len(prompts)
    portion_size = math.ceil(total_prompts / total_portions)
    start_index = portion_index * portion_size
    end_index = min(start_index + portion_size, total_prompts)
    portion_prompts = dict(list(prompts.items())[start_index:end_index])
    ...
    for file_basename, prompt in tqdm(portion_prompts.items()):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--total_portions", type=int, default=1, help="Total number of portions to divide the dataset into")
    parser.add_argument("--portion_index", type=int, default=0, help="Index of the portion to process (0-based)")
    args = parser.parse_args()
    
    model = load_model(args.model_name)
    infer_text_guided_vg_bench(model, fps=args.fps, total_portions=args.total_portions, portion_index=args.portion_index)
